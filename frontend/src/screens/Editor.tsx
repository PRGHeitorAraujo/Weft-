...
import type { Book, Edge, EdgeKind, InsightKind, Theme } from "../types";
...
export default function Editor({ insightId, books, themes, edges, onNavigateInsights, onCreated, onOpenInsight }: Props) {
    ...
    <main style={{ flex: 1, display: "flex", overflow: "hidden" }}>
      <div style={{ flex: 1, overflowY: "auto" }}>
        <div className="fade-up" style={{ maxWidth: 680, margin: "0 auto", padding: "44px 44px 96px" }}>
          ...
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Título (opcional)" style={{ width: "100%", marginTop: 26, padding: 0, border: "none", background: "transparent", fontFamily: "var(--prose)", fontSize: 26, fontWeight: 600, letterSpacing: "-0.015em", color: "var(--ink)" }} />
          <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="O que este trecho fez você pensar? Escreva a interpretação, não o resumo." rows={9} style={{ width: "100%", marginTop: 14, padding: 0, border: "none", background: "transparent", resize: "vertical", fontFamily: "var(--prose)", fontSize: 17, lineHeight: 1.7, color: "#2B2B33", minHeight: 220 }} />
          ...
          <div style={{ marginTop: 28, paddingTop: 26, borderTop: "1px solid var(--line)" }}>
            ...
            <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
              <select value={bookId} onChange={(e) => setBookId(e.target.value)} style={{ padding: "7px 10px", border: "1px solid var(--line)", borderRadius: 8, fontSize: 13, background: "#fff" }}>
                {books.map((b) => <option key={b.id} value={b.id}>{b.title}</option>)}
                <option value={NEW_BOOK}>+ novo livro</option>
              </select>
              {bookId === NEW_BOOK && (
                <>
              ...
              <input value={chapterLabel} onChange={(e) => setChapterLabel(e.target.value)} placeholder="Capítulo / localização (ex: Parte II, cap. 1)" style={{ flex: 1, minWidth: 200, padding: "7px 10px", border: "1px solid var(--line)", borderRadius: 8, fontSize: 13 }} />
            </div>
          </div>
          ...
          <div style={{ marginTop: 28 }}>
            ...
            <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
              {(Object.entries(TYPES) as [InsightKind, typeof TYPES[InsightKind]][]).map(([k, v]) => {
                const on = kind === k;
                return (
                  <button key={k} onClick={() => setKind(k)} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "6px 13px", borderRadius: 20, fontSize: 13, fontWeight: on ? 600 : 500, border: "1px solid " + (on ? v.color : "var(--line)"), background: on ? `color-mix(in oklab,${v.color} 7%,white)` : "#fff", color: on ? v.color : "var(--mut)" }}>
              ...
              <div style={{ flex: 1.2, minWidth: 250 }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {themes.map((t) => {
                    ...
                    return (
                      <button key={t.id} onClick={() => setThemeIds(on ? themeIds.filter((x) => x !== t.id) : [...themeIds, t.id])} style={{ padding: "5px 12px", borderRadius: 20, fontSize: 12.5, fontWeight: on ? 600 : 400, border: "1px solid " + (on ? "var(--accent)" : "var(--line)"), background: on ? "var(--accent)" : "#fff", color: on ? "#fff" : "var(--mut)" }}>
                        {t.label}
                      </button>
                    );
                  })}
